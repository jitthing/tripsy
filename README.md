...
- Set `RESEND_INBOUND_DOMAIN` to the receiving domain. Every user gets their own `imports-<token>@domain` address, shown at the top of the Inbox; mail sent there lands in that user's Inbox with no trip attached, and they assign a trip while reviewing. Per-trip addresses from the Imports tab still work and bind the import to that trip.
- `RESEND_INBOUND_ADDRESS` and `RESEND_INBOUND_OWNER_ID` are now an optional catch-all for a single-owner install: mail to that exact address is filed under that owner when no minted address matches. Leave them unset for a shared deployment. Mail matching nothing is acknowledged with `204` and logged at warn level with the recipients that missed.
...
- The worker that processes imports and calendar syncs runs one of two ways. `WORKER_MODE=loop` (the default) runs a background ticker inside the process, which suits a container host that keeps the process alive. `WORKER_MODE=cron` disables the ticker and expects a scheduler to call `GET /internal/cron/worker` instead, which is required on any host that suspends the process between requests — a background ticker there advances only while the process happens to be scheduled, and wall-clock deadlines expire during the gaps.
- The worker endpoint exists only when `CRON_SECRET` is set, and requires it as `Authorization: Bearer <secret>` or `?token=<secret>`. Each call reclaims stale leases, then drains both queues until they are empty or `CRON_BUDGET_SECONDS` (default 50) runs out; set that below the host's function timeout so the response is always written. Set `OPENROUTER_TIMEOUT_SECONDS` below it too, so one slow extraction cannot consume the whole budget.
- Opening the Inbox also calls `POST /v1/inbox/process`, which runs the worker against that user's own queue so a forward is handled while they are still looking at the screen. It is capped at three imports per call and `INBOX_BUDGET_SECONDS` (default 20), and claims only the caller's imports — the scheduled worker remains the guarantee.
