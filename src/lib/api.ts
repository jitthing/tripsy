...
export type ReservationImport = { id: string; tripId?: string; ownerId: string; sender: string; subject: string; receivedAt?: string; status: 'queued' | 'processing' | 'review' | 'approved' | 'discarded' | 'failed'; errorMessage: string; duplicateOfImportId?: string; usedLlm: boolean; createdAt: string }
